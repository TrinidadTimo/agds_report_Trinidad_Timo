...
LW_IN_F (the longwave radiation) is affected by a lot of missing data. As the longwave radiation is no crucial variable affecting GPP we decide to drop it (see model formulation in recipe).

### Imputatation
We decide to just drop rows with missing data instead of filling Na-observations through imputation to retain the rows (see recipe).

### Testing fo Zero-Variance-Predictors: 

```{r}
caret::nearZeroVar(daily_fluxes, saveMetrics = TRUE)
```

No predictors with near-zero variance. Also we have no categorical variables in our model, so no transformation by one-hot encoding needed for applying KNN.
...
In the distribution of our target variable no long tails can be observed. Therefore no further target engineering is needed.
...
Also, some of the predictors are not normally distributed. We decide to Box-Cox transform all predictors. This is also directly implemented in the recipe formulation. [Why?? Is normal distribution a condition for KNN? For regression, just the residuals have to be normally distributed for testing, right? Why are we doint this step.]
...
```
-> How to handle this issue? For temperature we can't just drop neg values, right?
...
Turning to the model comparison, this means that for the same variables the KNN approach (with k = 8) seems to be more prone to overfitting than the linear regression. Why is this the case? 
  ...
  < We are considering here just a regression model of order one (so linear). This means actually, we are just putting a straight line on our data. This is per se very inflexible. KNN doesn't have this linear constraint. 
  < Finally, we set k = 8 for our knn model. This seems quite low compared to our sample size of 1923 observations: For each estimation the model is just considering 0.4% of the whole sample. When also taking into account, that we use the mean afterwards which is prone to a high variance in the case of outliers, the knn model has an even greater disposition for overfitting. 
...
(I might have already answered this above, but for the sake of completeness I give it here another shot):
  < On the one hand, we are considering a regression model of order one, which is not very flexible (just putting a straight line on the data) on the other hand we are considering a KNN model with k=8, what means that each prediction is based on the closest 0.4% of the whole data, what makes it very flexible. 
...
3) Visualizing temporal variations of observed and modelled GPP

Regression Model: 

```{r}
plot_data <- daily_fluxes |> filter(SW_IN_F > 0, VPD_F > 0, TA_F > 0) |>
  drop_na(SW_IN_F, VPD_F, TA_F) # Because we undertook Box_Cox-Transformation on our predictors in the first place, which is not applicable on negative values. 
...
ggplot(data= plot_data, aes(x= TIMESTAMP, y= GPP_NT_VUT_REF)) +
  ...
  geom_point(aes(x= TIMESTAMP, y = gpp_pred), color = "blue", alpha = 0.3)
...
1) 
  - For k approaching 1, the R^2 for the estimate of the training data will go to equal 1, the Mae = 0. Vice versa, for the estimates of the test data, the R^2 will be low, the Mae high (as the model is extremely sensitive for the training sample (small bias there) -> high variance, high overfitting).
...
split <- rsample::initial_split(daily_fluxes, prop = 0.7, strata = "VPD_F") # Why do we want to maintain the distribution of the water-pressure deficite? 
...
```
  ...
  Note about the "error() exit": In previous trials I run into an error with the message stating "too many ties in knn". Following Chat-GPP, this error comes from a situation, in which more data points have the same or a smaller distance to our target than the given k (the number of observation we want to select). Therefore the algorithm does not know, which observations to choose. As this problem should (by the fact that we are using several predictors and regarding the given variability in them) just arise, when k reached an already relatively big value, I decide to stop the iteration. I think I got enough different k values to illustrate the dynamics in the model metrics it causes.
  
```{r}
eval_table <- data_frame("k" = integer(), "rsq_train" = numeric(), "rmse_train" = numeric(), "rsq_test" = numeric(), "rmse_test" = numeric())

source("./functions/eval_model_ex_2.R")
...
for (i in 1:nrow(na.omit(daily_fluxes_train_ex_2))) {
  ...
  result <- tryCatch({
    ...
    message(paste("Fehler bei k =", i, ":", e$message))
    break
  })
  
  # For the (normal) case of no error (k still small enough)
  if (exists("result")) {
