...
AIC_old = -1
  ...
  for (j in 2:(ncol(vars_clean))) {
    ...
    for (i in 2:ncol(vars_clean)) {
      ...
      mod <- lm(as.formula(paste(mod_pred[1], "~",paste(mod_pred[-1], collapse ="+"))), data = vars_clean) }
      R <- summary(mod)$r.squared
      if (R > R_max) {
        R_max <- R
        p <- i
      }
      if (i == ncol(vars_clean)) {
...
#print("The best model is:", mod_pred[1], "=", paste(mod_pred[-1], collapse = "+"), sep = " ")
