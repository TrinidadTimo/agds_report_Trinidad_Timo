...
-> What about non significant params? Why are they included but not being significant now? Problem of cut influence?
-> What if excluding TA_F??

-> Having identical predictors (TA_F_MDS, TA_F) included in the model is a good test -> They are very equal. So when having included one of them, the other should add (alomost) no additional variance, therefore not enhancing the R^2, so not/only at the very end, beeing included. 